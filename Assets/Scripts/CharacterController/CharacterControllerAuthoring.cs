...
public class CharacterControllerSystem : JobComponentSystem {
    ...
    struct CharacterControllerJob : IJobChunk {
        ...
        public unsafe void Execute (ArchetypeChunk chunk, int chunkIndex, int firstEntityIndex) {
            ...
            for (int i = 0; i < chunk.Count; i++) {
                ...
                HandleUserInput (ccComponentData, stepInput.Up, surfaceVelocity, ref ccInternalData, ref desiredVelocity, rotation);

                // Calculate actual velocity with respect to surface
                if (ccInternalData.SupportedState == CharacterSupportState.Supported) {
                    CalculateMovement (rotation, stepInput.Up, ccInternalData.IsJumping,
        ...
        private void HandleUserInput (CharacterControllerComponentData ccComponentData, float3 up, float3 surfaceVelocity,
            ref CharacterControllerInternalData ccInternalData, ref float3 linearVelocity, Rotation rotation) {
            ...
            float3 requestedMovementDirection = float3.zero; {
                ...
                if (haveInput) {
                    ...
                    // float3 worldSpaceMovement = math.rotate (rotation.Value, localSpaceMovement);
            ...
            // Turning
            {
                ...
                float3 forward = math.forward (CameraRotation);
                if (Input.Aimed) {
                    var targetDirection = new float3 (forward.x, 0f, forward.z);
                    var targetRotation = quaternion.LookRotation (targetDirection, up);
                    var velocity = math.mul (math.inverse (rotation.Value), targetDirection);
                    ccInternalData.CurrentRotationAngle += math.atan2 (velocity.x, velocity.z) * Mathf.Rad2Deg *
                        DeltaTime;
                }

                if (!Input.Aimed) { // normal type
                    float horizontal = ccInternalData.Input.Movement.x;
                    float vertical = ccInternalData.Input.Movement.y;
                    forward.y = 0.0f;
                    forward = math.normalize (forward);
                    float3 right = new float3 (forward.z, 0, -forward.x);
                    float3 targetDirection = forward * vertical + right * horizontal;
                    var isZero = targetDirection != float3.zero;
                    bool isMove = (horizontal != 0 || vertical != 0) && (isZero.x || isZero.y || isZero.z);
                    if (isMove) {
                        var targetRotation = quaternion.LookRotation (targetDirection, up);
                        var velocity = math.mul (math.inverse (rotation.Value), targetDirection);
                        ccInternalData.CurrentRotationAngle += math.atan2 (velocity.x, velocity.z) * Mathf.Rad2Deg *
        ...
        private void CalculateMovement (Rotation rotation, float3 up, bool isJumping,
    ...
    protected override void OnCreate () {
        ...
        characterInput = GetEntityQuery (typeof (CharacterControllerInput));
    }
    ...
    protected override JobHandle OnUpdate (JobHandle inputDeps) {
        ...
        var ccJob = new CharacterControllerJob () {
            ...
            Input = characterInput.GetSingleton<CharacterControllerInput> ()
        };
